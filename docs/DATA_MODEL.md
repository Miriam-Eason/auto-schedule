...
> 文档版本：1.1  
> 状态：开发基线；模式版本 4 已落地，Phase 4–7 均无模式变更（Phase 7 done，2026-09-02）
...
**Phase 7 实现状态：** 未新增表或迁移，最新模式版本仍为 4。备份是单个 `.duty-roster-backup` 文件，二进制包由固定 magic、版本 1 JSON 元数据和 rusqlite 在线备份 API 生成的 SQLite 一致性快照组成。元数据包含应用版本、模式版本、UTC 导出时间、SQLite 字节数、FNV-1a 64 校验值，以及教师/学期/月/日期/值班记录数量摘要。

恢复预览会验证包格式、长度、校验值、模式版本、`PRAGMA integrity_check`、`pragma_foreign_key_check` 和摘要一致性，并生成与包内容绑定的恢复令牌，阻止预览后文件被替换。确认恢复后先创建当前数据库安全副本，再通过 SQLite backup API 写入已验证快照；写入或复核失败时恢复安全副本。成功后再次检查模式、完整性、外键和摘要，前端重新加载所有数据。当前仅接受模式版本 4 的备份，不兼容版本会在覆盖前明确拒绝。
